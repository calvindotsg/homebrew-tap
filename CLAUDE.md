...
- **Python virtualenv formula** — see `Formula/cc-menubar.rb` (SwiftBar plugin), `Formula/mac-upkeep.rb` (launchd service). Explicit `resource` stanzas required per `Language::Python::Virtualenv`; generate with `brew update-python-resources` (**not** `poet` — see Non-Obvious Constraints for why it silently stopped working).
...
- **Explicit third-party `formula:` list in livecheck** — scope the cron to non-Calvin-owned formulas by enumerating them (currently `opensrc`, `cloudflare-cf` and `grok-bot-cli` in the `bump` job, the two npm ones behind the 24 h publish cooldown in its `lane` step; `pymarkdownlnt` has its own `bump-python` job). Avoids races with the `repository_dispatch` path used by Calvin-owned formulas.
...
- **Homebrew 6.0+ tap trust — enforcement depends on how you NAME things.** `HOMEBREW_REQUIRE_TAP_TRUST` defaults to true and `brew tap` does **not** confer trust; only `brew trust` does, writing `~/.homebrew/trust.json`. But `Homebrew::Trust.explicitly_allowed?` (`trust.rb`) inspects **`ARGV`**, so behaviour splits three ways — verified empirically 2026-08-06 by untrusting the tap locally:
  ...
  - **CI**: workflow steps here use qualified names and so do not strictly require trust, but both cask jobs call `brew trust --tap calvindotsg/tap` (idempotent, exit 0) so a future bare-token or bulk command cannot reintroduce the silent-skip mode. The `bump` job gets this free, but from `Homebrew/actions/setup-homebrew`, not from the bump action: its `main.sh` runs `brew trust "$HOMEBREW_TAP_NAME"` for any tap that is not `homebrew/*`, and that is where the `==> Trusting calvindotsg/tap tap...` line in the job log comes from. `action-homebrew-bump-formula` grew its own `brew trust` in v8, but only on the branch that handles a `tap:` input — this workflow passes fully-qualified formula names instead, so that line never runs here.
...
- **Do not use `homebrew-pypi-poet`, and do not let resource regeneration fail soft.** Modern setuptools no longer ships `pkg_resources`, so `poet` dies at import. The tap ran it that way in production and the old `::warning::`-and-continue fallback hid it: bumps kept succeeding while the resource pins silently froze — a `mac-upkeep` run resolved `typer 0.27.1` / `rich 15.0.0` / `pygments 2.21.0` in its venv and committed a formula still pinning `0.24.1` / `14.3.3` / `2.20.0`, so users stopped receiving upstream fixes with nothing looking broken. `update-formula.yml` now uses `brew update-python-resources --print-only --ignore-main-package-cooldown --package-name=<pkg> --version=<v>` and **fails the job** if it errors. Note `--package-name`: it lets Homebrew resolve a formula whose `url` is a GitHub tarball, provided the package is on PyPI. `--ignore-main-package-cooldown` exempts only our own just-released package; the dependencies still respect the cooldown.
- **`brew bump-formula-pr` applies no release cooldown** — `brew bump` does (`dev-cmd/bump.rb`), but the per-formula command has none, and `dawidd6/action-homebrew-bump-formula` drives `livecheck --newer-only` straight into it. With auto-merge that is a straight line from an npm publish to a user's machine: `cf@0.7.0` went published → merged in **11 hours**. `livecheck.yml`'s `bump` job therefore checks the npm publish timestamp itself and defers a release younger than 24h to a later run; an unreachable registry defers too, since "don't bump" is the safe direction.
...
- **`persist-credentials: false` breaks a bare `git push`.** `actions/checkout`'s default persists a token for the steps that follow — in `.git/config`'s `http.extraheader` up to v5, and from v6 in a separate file under `$RUNNER_TEMP` wired in with an `includeIf` — and a step that just runs `git push` depends entirely on it. Setting the flag without re-authenticating the push in the same change is half a fix that fails at the last step.
- **`brew test-bot` does not cover the credential-bearing workflows.** `tests.yml` is the only CI a Dependabot action-pin bump gets, and it exercises `checkout`, `cache`, and — only when the PR changed a formula — `upload-artifact`. Nothing in `livecheck.yml`, `livecheck-casks.yml`, `update-formula.yml` or `update-cask.yml` ever runs on a pull request, so a green pin bump says nothing about the pins that actually matter. Validate those lanes with a dispatch on the bump branch (`gh workflow run <workflow> --ref <branch>`) before merging. `update-formula.yml` re-dispatched at a formula's *current* version and URL is an idempotent end-to-end smoke test of the whole two-job hand-off: both publishing jobs bail with "No changes … nothing to open" when the rewritten file matches what is already committed.
- **The artifact hand-off fails closed on a digest mismatch from `download-artifact` v8 on.** `digest-mismatch` defaults to `error` (v4–v7 only warned), so the file crossing from the credential-free job to the publishing one is checked against the digest recorded at upload *before* either workflow's own url/sha256 verification runs. Do not set it back to `warn`. Everything from v5 up also needs an Actions runner >= 2.327.1 for the node24 runtime — free on GitHub-hosted runners, a real constraint if a self-hosted one is ever added.
